...
class TrimodalDataset(Dataset):
    ...
    def __init__(self, data_prefix):
        ...
        # This loads the labels that correspond to the successfully processed files.
        original_labels = np.load(os.path.join(self.data_dir, 'labels.npy'))
        
        # This loads the original file indices of the successfully processed files.
        self.indices = np.load(os.path.join(self.data_dir, 'indices.npy'))
        self.num_samples = len(self.indices)

        # --- FIX: The `original_labels` array is already the correct, filtered list. ---
        # No further indexing is needed here.
        ...
        # Apply mapping to the correctly filtered labels
        self.labels = np.array([self.seven_to_five_map.get(label, -1) for label in valid_original_labels])
        
        # The lengths of self.indices and self.labels should now match perfectly.
...
def plot_confusion_matrix(labels, preds, save_path="confusion_matrix_trimodal.png"):
    ...
    plt.ylabel('Actual'); plt.xlabel('Predicted'); plt.title('Trimodal Confusion Matrix (5 Classes)')
...
if __name__ == "__main__":
    ...
    train_dataset = TrimodalDataset('train')
    ...
    model = TrimodalFusionModel(num_classes=num_classes).to(device)
    ...
    for epoch in range(EPOCHS):
        model.train()
        running_loss = 0.0
        for text_batch, audio_batch, vision_batch, labels_batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS} [Training]"):
            text_batch, audio_batch, vision_batch, labels_batch = text_batch.to(device), audio_batch.to(device), vision_batch.to(device), labels_batch.to(device)
            optimizer.zero_grad()
        ...
        all_preds = []
        with torch.no_grad():
            for text_batch, audio_batch, vision_batch, labels_batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{EPOCHS} [Validation]"):
                text_batch, audio_batch, vision_batch, labels_batch = text_batch.to(device), audio_batch.to(device), vision_batch.to(device), labels_batch.to(device)
                outputs = model(text_batch, audio_batch, vision_batch)
                loss = criterion(outputs, labels_batch)
        ...
        # --- MODIFICATION: Early stopping based on F1 score with a performance threshold ---
        if f1 > best_val_f1:
            best_val_f1 = f1
            patience_counter = 0
            torch.save(model.state_dict(), 'best_trimodal_model.pth')
            print("✅ New best model saved!")
        # Only start the patience counter if the model has reached a reasonable performance
        elif best_val_f1 > 0.60:
            patience_counter += 1
            if patience_counter >= 3:
                print("Early stopping triggered after reaching >60% F1.")
                break
    
    print("✅ Training complete.")

    print("\nLoading best model for visualization...")
    model.load_state_dict(torch.load('best_trimodal_model.pth', weights_only=True))
    
    model.eval()
    all_labels_cm, all_preds_cm = [], []
    with torch.no_grad():
        for text_batch, audio_batch, vision_batch, labels_batch in tqdm(val_loader, desc="Generating predictions for Confusion Matrix"):
            text_batch, audio_batch, vision_batch = text_batch.to(device), audio_batch.to(device), vision_batch.to(device)
            outputs = model(text_batch, audio_batch, vision_batch)
            _, predicted = torch.max(outputs.data, 1)
